...
REAL(EB) :: RDT,DTMP,QDXKF,QDXKB,RR,RFACF,RFACB,RFACF2,RFACB2, &
            ...
            ZZ_G(1:N_TRACKED_SPECIES),CP1,CP2,DENOM,KAPPA_VAL,DIST,REFLECTIVITY1,REFLECTIVITY2,REFRACTION
...
IF (SF%INTERNAL_RADIATION) THEN
   ...
   DO I=1,NWP
      IF (ML%T_SOURCE>0) THEN
         ...
         DO N=1,SF%N_MATL
            ...
            ML  => MATERIAL(SF%MATL_INDEX(N))
            KAPPA_VAL=(ML%KAPPA_COEF(1)*EXP(ML%KAPPA_COEF(2)*ML%T_SOURCE)+ML%KAPPA_COEF(3)*EXP(ML%KAPPA_COEF(4)*ML%T_SOURCE)) &
                *(ML%KAPPA_COEF(5)*DIST**ML%KAPPA_COEF(6)+ML%KAPPA_COEF(7))
            VOLSUM = VOLSUM + ONE_D%MATL_COMP(N)%RHO(I)/ML%RHO_S
            KAPSUM = KAPSUM + ONE_D%MATL_COMP(N)%RHO(I)*KAPPA_VAL/ML%RHO_S
         ENDDO
   ...
   ! solution inwards
   IF (ML%T_SOURCE>0) THEN
      REFLECTIVITY1 = 1-B1%EMISSIVITY
      REFRACTION=-0.03058*EXP(2.813*B1%EMISSIVITY)+25.75*EXP(-2.868*B1%EMISSIVITY)
      REFLECTIVITY2 = 0.7148*EXP(0.08737*REFRACTION)-9.627*EXP(-2.528*REFRACTION)
      RFLUX_UP = B1%Q_RAD_IN*(1-REFLECTIVITY1)/B1%EMISSIVITY + REFLECTIVITY2*Q_RAD_OUT_OLD/(1-REFLECTIVITY2)
      DO I=1,NWP
         RFLUX_DOWN =  ( RFLUX_UP + TWO_DX_KAPPA_S(I)*(REFRACTION**2)*SIGMA*ONE_D%TMP(I)**4 ) / (1._EB + TWO_DX_KAPPA_S(I))
      ...
      DO I=NWP,1,-1
         ...
         RFLUX_UP = RFLUX_DOWN
      ENDDO
      Q_RAD_OUT = (1-REFLECTIVITY2)*RFLUX_DOWN
   ELSE
